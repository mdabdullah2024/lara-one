...
class NoticeBoardModel extends Model
{
    ...
    {
        $return = self::select('notice_board.*','users.name as created_by_name','users.last_name as created_by_lastname')
   ...
   $return = $return->orderBy('notice_board.id','desc')
    ...
    static public function NoticeboardParentCount($message_to)
    {
        $return = self::select('notice_board.*','users.name as created_by_name','users.last_name as created_by_lastname')
                        ->join('users','users.id','=','notice_board.created_by');
       $return = $return->count();
